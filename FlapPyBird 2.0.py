...
bg.img = pygame.transform.scale2x(pygame.image.load(os.path.join("imgs","bg.png")).convert_alpha(), (600,900))
...
class Bird:
    ...
    def draw(self, win):
        ...
        if self.tilt <= -80:
            self.img = self.IMGS[1]
            self.img_count = self.ANIMATION_TYPE*2
        ...
        if not lost:
            ...
            if start:
                ...
                for pipe in pipes:
                    ...
                    if not pipe_passed and pipe.x < bird.x:
                        pipe.passed = True
                        add_pipe = True
